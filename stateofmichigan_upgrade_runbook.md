...
```
### Certificate Manager Migration (For 5.3.x Upgrades)

When upgrading to CPD 5.3.x from an earlier version, you must migrate from IBM Certificate Manager to Red Hat OpenShift cert-manager before upgrading IBM Software Hub.

**Note**: This is a one-time migration. If you are already on 5.3.x, this step has been completed and can be skipped.

#### Certificate Manager Migration (IBM to Red Hat)

**Overview**: CPD 5.3.x requires migration from IBM Certificate Manager to Red Hat OpenShift cert-manager Operator. This must be completed BEFORE upgrading IBM Software Hub.

**Reference**: [IBM Documentation - Migrating to Red Hat OpenShift Certificate Manager](https://www.ibm.com/docs/en/software-hub/5.3.x?topic=upgrading)

---

### Step 1: Backup Existing Certificates

**Reference**: [Backing up your existing certificates](https://www.ibm.com/docs/en/software-hub/5.3.x?topic=upgrading)

```bash
# Create a temporary project for validation
oc new-project cert-mgr-test

# Backup Issuer custom resources
oc get issuers.cert-manager.io -A -o yaml > issuer_list.yaml

# Backup Certificate custom resources
oc get certificates.cert-manager.io -A -o yaml > certificate_list.yaml

# Verify IBM Certificate manager is working correctly
...
metadata:
  name: verify-issuer
  namespace: cert-mgr-test
spec:
  selfSigned: {}
EOF

# Apply the backed up issuers
...
oc get certificate verify-certificate -n cert-mgr-test -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}'
# Should return: True

# Clean up test project
oc delete project cert-mgr-test
```

---

### Step 2: Uninstall IBM Certificate Manager

**Reference**: [Uninstalling IBM Certificate Manager](https://www.ibm.com/docs/en/software-hub/5.3.x?topic=upgrading)

```bash
# Delete IBM Certificate Manager operator
cpd-cli manage delete-olm-artifacts \
  --cpd_operator_ns=${PROJECT_CPD_INST_OPERATORS} \
  --components=ibm-cert-manager

# Verify IBM Certificate Manager is removed
oc get pods -n ${PROJECT_CERT_MANAGER}
# Should return: No resources found
```

---

### Step 3: Mirror Red Hat OpenShift cert-manager Images (Air gapped Only)

**Reference**: [Mirroring Red Hat OpenShift certificate manager images](https://www.ibm.com/docs/en/software-hub/5.3.x?topic=upgrading)

```bash
# Set environment variables for mirroring
export PRIVATE_REGISTRY=${PRIVATE_REGISTRY_LOCATION}
export PRIVATE_REGISTRY_USER=${PRIVATE_REGISTRY_PULL_USER}
export PRIVATE_REGISTRY_PASSWORD=${PRIVATE_REGISTRY_PULL_PASSWORD}

# Login to registries
podman login ${PRIVATE_REGISTRY} -u ${PRIVATE_REGISTRY_USER} -p ${PRIVATE_REGISTRY_PASSWORD}
podman login registry.redhat.io -u ${RH_REGISTRY_USER} -p ${RH_REGISTRY_PASSWORD}

# Mirror cert-manager images
oc image mirror \
  registry.redhat.io/cert-manager/cert-manager-operator-bundle:v1.13.1 \
  ${PRIVATE_REGISTRY}/cert-manager/cert-manager-operator-bundle:v1.13.1

oc image mirror \
  registry.redhat.io/cert-manager/cert-manager-rhel9-operator:v1.13.1 \
  ${PRIVATE_REGISTRY}/cert-manager/cert-manager-rhel9-operator:v1.13.1

oc image mirror \
  registry.redhat.io/cert-manager/cert-manager-controller-rhel9:v1.13.1 \
  ${PRIVATE_REGISTRY}/cert-manager/cert-manager-controller-rhel9:v1.13.1

oc image mirror \
  registry.redhat.io/cert-manager/cert-manager-webhook-rhel9:v1.13.1 \
  ${PRIVATE_REGISTRY}/cert-manager/cert-manager-webhook-rhel9:v1.13.1

oc image mirror \
  registry.redhat.io/cert-manager/cert-manager-cainjector-rhel9:v1.13.1 \
  ${PRIVATE_REGISTRY}/cert-manager/cert-manager-cainjector-rhel9:v1.13.1
```

---

### Step 4: Install Red Hat OpenShift cert-manager Operator

**Reference**: [Installing the cert-manager Operator](https://www.ibm.com/docs/en/software-hub/5.3.x?topic=upgrading)

```bash
# Create cert-manager-operator namespace
oc create namespace cert-manager-operator

# Create ImageContentSourcePolicy for air gapped environments
cat <<EOF | oc apply -f -
...
spec:
  ...
  - mirrors:
    - ${PRIVATE_REGISTRY}/cert-manager
    source: registry.redhat.io/cert-manager
EOF

# Wait for nodes to apply the ImageContentSourcePolicy
oc get mcp -w
...
# Wait for cert-manager operator to be ready
oc wait --for=condition=Ready pod -l app.kubernetes.io/name=cert-manager-operator -n cert-manager-operator --timeout=300s
...
oc get pods -n cert-manager | grep -v Running
# Should return no results

# Check cert-manager version
oc get deployment cert-manager -n cert-manager -o jsonpath='{.spec.template.spec.containers[0].image}'
```
...
```bash
# Certificates will be automatically recreated by CPD operators during upgrade
# No manual intervention required

# After upgrade, verify certificates are issued
oc get certificates -n ${PROJECT_CPD_INST_OPERANDS}
oc get certificaterequests -n ${PROJECT_CPD_INST_OPERANDS}

# Check certificate status
oc get certificates -n ${PROJECT_CPD_INST_OPERANDS} -o wide
...
**⚠️ CRITICAL**: This migration MUST be completed successfully before proceeding with IBM Software Hub upgrade.
...
**Private Registry**: stateofmichigan.example.com:5000
...
**Current OCP Version**: 4.16
**Target OCP Version**: 4.16.4
**Upgrade Path**: 4.16 → 4.16.4

**Important Notes**:
- ⚠️  Current OCP 4.16 needs patch upgrade to 4.16.4
